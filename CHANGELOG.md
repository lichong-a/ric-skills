...
- Renamed upstream skills to `ric-*` install names.
- Added a RIC strengthening layer to upstream-derived skills.
- Added `ric-admin-console` for Chinese enterprise admin systems.
- Added RIC engineering skills for agent rules, infrastructure safety, Node/pnpm, backend services, data pipelines, API design, testing, code review, deployment, and documentation.
- Added `NOTICE.md`, `skill.ps1`, RIC `skill.sh`, and `skills/llms.txt`.
